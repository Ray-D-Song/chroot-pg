...
# adding the HTTPS-only PGDG repository, otherwise apt cannot verify PGDG.
...
chroot "$ROOTFS" /bin/bash -ec '
  export DEBIAN_FRONTEND=noninteractive
  apt-get update
  apt-get install -y --no-install-recommends locales postgresql-17="'"$POSTGRES_PACKAGE_VERSION"'" postgresql-client-17="'"$POSTGRES_PACKAGE_VERSION"'"
  locale-gen C.UTF-8
